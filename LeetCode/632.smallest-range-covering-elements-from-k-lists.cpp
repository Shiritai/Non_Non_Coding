...
public:
    ...
    vector<int> smallestRange(vector<vector<int>>& nums) {
        ...
        eroikoDS::i_heap<it_v> ihp_min(len,
            [](it_v& a, it_v& b) -> bool { return *a < *b; });
        eroikoDS::i_heap<it_v> ihp_max(len,
            [](it_v& a, it_v& b) -> bool { return *a > *b; });
        ...
        for (int i = 0; i < len; ++i) {
            ...
            l = min(l, front);
            r = max(r, v.back());
            ihp_min.push(v.begin(), i);
            ihp_max.push(v.begin(), i);
        }

        while (true) {
            int local_l = *ihp_min.top(),
                local_r = *ihp_max.top();
            if (local_r - local_l < r - l) {
                r = local_r;
                l = local_l;
            }
            int adjust_id = ihp_min.top_id();
            auto to_adjust = ihp_min.top() + 1;
            if (to_adjust != nums[adjust_id].end()) {
                ihp_min.adjust(adjust_id, to_adjust);
                ihp_max.adjust(adjust_id, to_adjust);
            }
            else 
                break;
        }
