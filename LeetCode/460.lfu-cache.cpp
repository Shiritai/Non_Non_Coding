...
public:
     ...
     * 
     * NOTE: Please always call the "adjust" method 
     * after assigning a value using subscription 
     * operator in order to adjust the element of this index 
     */
    T & operator[](int i);
...
template <typename T>
i_heap<T>::i_heap(int cap, cmp_fn lessThan): 
    cap(cap + 1), sz(0), 
    arr((T *) malloc(sizeof(T) * (cap + 1))), 
    i_ls(new int [cap + 1]),
    r_ls(new int [cap + 1]), 
    lessThan(lessThan) { // Deliberately left index 0 empty
...
template <typename T>
int i_heap<T>::capacity(){ return cap - 1; }
...
public:
    LFUCache(int capacity): 
        cache(capacity, 
            [](entry& e1, entry& e2) -> bool {
                return e1.cnt != e2.cnt ? e1.cnt < e2.cnt :
                    e1.age < e2.age; }), 
        global_age(0) {
    ...
    int get(int key) {
        cout << '(' << ++op_cnt << ") get [" << key << ']' << endl;
        auto ind = index.find(key);
        if (ind != index.end()) {
            ++cache[ind->second].cnt;
            cache[ind->second].age = global_age++;
            cache.adjust(ind->second);
            cout << "key exist with value " << cache[ind->second].val << endl;
            return cache[ind->second].val;
        }
        if (!cache.empty())
            cout << "top is " << cache.top().it->first << ": " << cache.top().val << endl;
        cout << "key DNE\n";
        return -1;
    }
    ...
    void put(int key, int value) {
        ...
        entry to_add { 
            ...
            .age = global_age++, 
            .it = index.end() };
        cout << '(' << ++op_cnt << ") put [" << key << ": " << value << ']' << endl;
        if (!cache.empty())
            cout << "top is " << cache.top().it->first << ": " << cache.top().val << endl;
        for (auto i: index)
            cout << i.first << ": " << cache[i.second].val << ", ";
        cout << endl;
        if ((to_add.it = index.find(key)) != index.end()) { // exist -> update
            cout << "exist " << key << endl;
            cache.adjust(index[key], to_add);
            return;
        }
        if (cache.size() == cache.capacity()) { // remove LFU
            cout << "is full, pop " << cache.top().it->first << ": " << cache.top().val << endl;
            ind = cache.top_id(); // index in i_heap to remove and insert entry
            index.erase(cache[ind].it);
            cache.adjust(ind, to_add);
        }
        else {
            ind = cache.size(); // index in i_heap to insert entry
            cache.push(to_add, ind);
        }
        index[key] = ind;
        cache[ind].it = index.find(key); // save iterator
    }
};
// @lc code=end
